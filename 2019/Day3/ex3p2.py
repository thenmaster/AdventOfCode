...
pair_str = [repr(pair) for pair in pairs]

mov_dic = {key : 0 for key in pair_str}
...
def mov_cord(coord, mov, line):
	...
	if direction == "U":
		for i in range(0, delta):
			if repr([moves[-1][0], moves[-1][1] + 1]) in mov_dic:
				mov_dic[repr([moves[-1][0], moves[-1][1] + 1])] += (mov1 if line else mov2)
			moves += [[moves[-1][0], moves[-1][1] + 1]]
			if line:
				mov1+=1
			else:
				mov2+=1
		...
		for i in range(0, delta):
			if repr([moves[-1][0] - 1, moves[-1][1]]) in mov_dic:
				mov_dic[repr([moves[-1][0] - 1, moves[-1][1]])] += (mov1 if line else mov2)
			moves += [[moves[-1][0] - 1, moves[-1][1]]]
			if line:
				mov1+=1
			else:
				mov2+=1
...
for i in range(0, len(wire1)): # assuming wires have same lenght, hopefully that is always true
	cur1 = wire1pos[-1]
	cur2 = wire2pos[-1]

	wire1pos += mov_cord(cur1, wire1[i], True)
	wire2pos += mov_cord(cur2, wire2[i], False)

print(mov_dic)
print(mov_dic[min(mov_dic.keys(), key=(lambda k: mov_dic[k]))])